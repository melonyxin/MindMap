...
QDomElement PolylineStyle::focusToDom(QDomDocument *doc){
...
QDomElement PolylineStyle::nodeToDom(QDomDocument *doc,Node *node){
    ...
    if(!node->isMasterNode()){
        ...
        QPointF parentStart = (parent->getCustomRect().bottomRight()+parent->getCustomRect().topRight())/2 - node->pos();
        QRectF itemRect = node->getCustomRect();
        QPointF p = (itemRect.bottomLeft()+itemRect.topLeft())/2;
        QPointF c1 = QPointF(parentStart.x()+50,parentStart.y());
        QPointF c2 = QPointF(c1.x(),p.y());
        QString pathString = "M"+QString::number(parentStart.x())+","+QString::number(parentStart.y())
                              +" L"+QString::number(c1.x())+","+QString::number(c1.y())
                              +" L"+QString::number(c2.x())+","+QString::number(c2.y())
                              +" L"+QString::number(p.x())+","+QString::number(p.y());
